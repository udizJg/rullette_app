/**
 * Ruleta 6 NIU locales — misma lógica que Chicureo (ruleta 3).
 * Mantener alineado con NIU_SEGMENT_PRIZES en server/lib/niuSpinService.js.
 */
const WHEEL_SVG_URL = '/assets/Ruleta-NIU-locales-.svg'
...
const STORAGE_ANON = 'ruleta6_niu_anon_id'
const POLL_MS = 5000
...
function mountWheelSvg() {
  ...
  return fetch(WHEEL_SVG_URL, { cache: 'default' })
    ...
    .then(markup => {
      ...
      svg.setAttribute('aria-label', 'Ruleta NIU locales')
    ...
    .catch(err => {
      console.error('No se pudo incrustar el SVG NIU:', err?.message || err)
      const img = document.createElement('img')
      img.className = 'wheel-img wheel-img--fallback'
      img.alt = 'Ruleta NIU locales'
...
async function fetchStatus() {
  ...
  const res = await fetch(`/api/niu/status?${q.toString()}`)
  return res.json()
}
...
function applyStatusToUi(st) {
  ...
  if (st.code === 'not_configured') {
    statusLine.textContent = 'NIU locales no está configurado en el servidor.'
    spinBtn.disabled = true
    resultMsg.textContent = ''
    clearPrizeAnnouncement()
    return
  ...
  if (st.code === 'inactive_campaign') {
    statusLine.textContent =
      st.reason === 'before_campaign'
        ? 'Las activaciones NIU aún no comienzan.'
        : st.reason === 'after_campaign'
          ? 'Las activaciones NIU finalizaron.'
          : st.reason === 'not_activation_day'
            ? 'Hoy no hay activación NIU.'
            : 'Campaña NIU no disponible.'
    spinBtn.disabled = true
    resultMsg.textContent = ''
    clearPrizeAnnouncement()
    return
...
async function postSpin(idempotencyKey) {
  const anonId = getOrCreateAnonId()
  const fpId = anonId ? '' : softFingerprint()
  const res = await fetch('/api/niu/spin', {
    method: 'POST',
