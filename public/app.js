...
const SEGMENTS = SEGMENT_ORDER_CLOCKWISE.map((prize, index) => ({
  ...
  centerDeg: normalizeAngle(
    POINTER_ANGLE_DEG + SEGMENT_STEP_DEG / 2 + index * SEGMENT_STEP_DEG,
  ),
}));
...
function animateToPrize(prize) {
  ...
  const desiredNormalized = normalizeAngle(
    POINTER_ANGLE_DEG - segmentCenterLocalDeg,
  );
...
function applyStatusToUi(st) {
  ...
  if (outside) {
    resultMsg.textContent =
      'La ruleta solo está disponible en el horario configurado para hoy.';
    spinBtn.disabled = true;
    clearPrizeAnnouncementClasses();
    return;
  }
...
spinBtn.addEventListener('click', async () => {
  ...
  if (!ok && data.code !== 'sold_out' && data.code !== 'already_played') {
    lockResultMessageUntil = 0;
    resultMsg.textContent =
      data.message || 'Error de red. Reintentá con el mismo intento.';
    spinBtn.disabled = false;
    return;
  }
