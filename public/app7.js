/**
 * Ruleta 7 NIU25 — merch con inventario diario + 30% «Sigue participando».
 * Mantener alineado con NIU25_SEGMENT_OUTCOMES en server/lib/niu25SpinService.js.
 */
const WHEEL_SVG_URL = '/assets/niu25.svg'
...
const STORAGE_ANON = 'ruleta7_niu25_anon_id'
const POLL_MS = 5000
...
function mountWheelSvg() {
  ...
  return fetch(WHEEL_SVG_URL, { cache: 'default' })
    ...
    .then(markup => {
      ...
      svg.setAttribute('aria-label', 'Ruleta NIU25')
    ...
    .catch(err => {
      console.error('No se pudo incrustar el SVG NIU25:', err?.message || err)
      const img = document.createElement('img')
      img.className = 'wheel-img wheel-img--fallback'
      img.alt = 'Ruleta NIU25'
...
async function fetchStatus() {
  ...
  const res = await fetch(`/api/niu25/status?${q.toString()}`)
  return res.json()
}
...
function applyStatusToUi(st) {
  ...
  if (st.code === 'not_configured') {
    statusLine.textContent = 'NIU25 no está configurado en el servidor.'
    spinBtn.disabled = true
    resultMsg.textContent = ''
    clearPrizeAnnouncement()
    return
  ...
  if (st.code === 'inactive_campaign') {
    statusLine.textContent =
      st.reason === 'before_campaign'
        ? 'La activación NIU25 aún no comienza.'
        : st.reason === 'after_campaign'
          ? 'La activación NIU25 finalizó.'
          : st.reason === 'not_activation_day'
            ? 'Hoy no hay activación NIU25.'
            : 'Campaña NIU25 no disponible.'
    spinBtn.disabled = true
    resultMsg.textContent = ''
    clearPrizeAnnouncement()
    return
...
async function postSpin(idempotencyKey) {
  const anonId = getOrCreateAnonId()
  const fpId = anonId ? '' : softFingerprint()
  const res = await fetch('/api/niu25/spin', {
    method: 'POST',
