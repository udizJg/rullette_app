...
import { campaignDayIndex, isWithinWindow } from './lib/schedule.js';
import { createStore } from './lib/store.js';
import { PRIZE_LABELS, availablePrizeKeys } from './lib/prizes.js';
import {
  applySpinMutation,
  participantKeyFromBody,
} from './lib/spinService.js';
...
app.get('/assets/wheel.svg', (_req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send(`No se pudo leer el SVG: ${(e && e.message) || 'error'}`);
  }
});

function nowContext() {
  const nowUtc = new Date();
  const camp = campaignDayIndex(
    nowUtc,
  ...
  return { ok: true, nowInTz, dayIndex, dayKey, win, camp };
}
...
function ensureDayInventory(state, dayKey, dayIndex) {
  if (!state.days) state.days = {};
  if (!state.days[dayKey]) {
    const inv = { ...config.defaultLimits };
    if (!isToteEligibleDay(dayIndex)) inv.tote = 0;
    state.days[dayKey] = {
      inventory: inv,
      spins: {},
      idempotency: {},
    ...
    // Reconciliamos tote con la regla actual por si el estado persistido
    // fue creado con reglas anteriores.
    if (!isToteEligibleDay(dayIndex)) {
...
function getDaySnapshot(dayKey, dayIndex) {
  const state = store.readSync();
  const d = state.days?.[dayKey];
  const toteEligibleForDay = isToteEligibleDay(dayIndex);
  if (!d) {
    const inv = { ...config.defaultLimits };
...
app.get('/api/status', (req, res) => {
  ...
  const effectiveWin = isDevelopment
    ? { ...win, active: true, reason: null }
    : win;
  const snap = getDaySnapshot(dayKey, dayIndex);
  const pool = availablePrizeKeys(
    snap.inventory,
  );
...
app.post('/api/spin', (req, res) => {
  ...
  if (isDevelopment) {
    try {
      ...
      const beforeInventory = {
        ...ensureDayInventory(shadowState, dayKey, dayIndex),
      };
      const result = applySpinMutation(shadowState, {
        ...
        toteEligibleForDay: isToteEligibleDay(dayIndex),
        enforceOnePerParticipant,
      });
      const afterInventory = result.payload?.remaining
        ? { ...result.payload.remaining }
        : { ...beforeInventory };
      const prize = result.payload?.prize || null;
      const deltaCheck = validateInventoryDelta(
        beforeInventory,
        afterInventory,
        prize,
      );
  ...
  store
    .runMutation((state) => {
      const beforeInventory = { ...ensureDayInventory(state, dayKey, dayIndex) };
